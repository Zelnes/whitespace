...
module.exports = class Whitespace {
  constructor () {
    ...
    this.setOfOldCursorPositions = new Set()
  ...
  handleEvents (editor) {
    ...
    let editorCursorPotisionChanged = editor.onDidChangeCursorPosition(event => {
       ...
       if (buffer.lineLengthForRow(row) && buffer.isRowBlank(row))
           this.setOfOldCursorPositions.add(row)
    })

    let editorTextInsertedSubscription = editor.onDidInsertText(event => {
        this.setOfOldCursorPositions.forEach(function(row) {
      ...
      let scopeDescriptor = editor.getRootScopeDescriptor()
      // console.log("MGH ====== ", event.range.start.row)
    ...
    this.subscriptions.add(editorCursorPotisionChanged)
